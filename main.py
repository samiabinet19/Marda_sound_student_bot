...
def init_db():
    ...
    # 🟢 Event Loop Blocking የሚያስቀረው የWAL Mode ማስተካከያ
...
def _get_paid_users_only():
    ...
    return [{
        ...
        'batch': r[3], 'payment_date': r[4], 'payment_status': r[5]
    } for r in rows]
...
def _get_users_by_batch(batch_name: str):
    ...
    return [{
        'user_id': r[0], 'name': r[1], 'phone': r[2], 
        'payment_status': r[3], 'payment_date': r[4], 'is_banned': r[5]
    } for r in rows]
...
def _get_all_users():
    ...
    conn.close()
    return [{
        'user_id': r[0], 'name': r[1], 'phone': r[2], 'batch': r[3],
        'payment_status': r[4], 'balance': r[5], 'is_banned': r[6], 'payment_date': r[7]
    } for r in rows]

def _get_approved_users_for_check():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
# 🟢 Thread Safe Async Wrappers (Event Loop እንዳይዘጋ ይከላከላሉ)
...
async def get_approved_users_for_check():
    return await asyncio.to_thread(_get_approved_users_for_check)

# ------------------ 4. የወርሃዊ ክፍያ ማስታወሻ ------------------
async def check_expired_payments_logic(bot):
    approved_users = await get_approved_users_for_check()
    today = datetime.now()
...
# ------------------ 7. Admin Dashboard & Batch Handlers ------------------
...
if __name__ == '__main__':
    ...
    print("🚀 ቦቱ በሙሉ አቅሙና ያለ ምንም Event Loop መዘጋት ስራ ጀምሯል...")
    app.run_polling()
